...
SELECT
    ...
    -- 3-week moving average of patient satisfaction (current + 2 prior weeks)
    AVG(patient_satisfaction) OVER (
        PARTITION BY service
        ORDER BY week
        ROWS BETWEEN 2 PRECEDING AND CURRENT ROW
    ) AS moving_avg_sat_3w,

    -- Difference between current week admissions and service average admissions
    patients_admitted
